import React from 'react'
import Styles from './icon-styles.scss'

export enum IconType {
    thumbDown = 'data:image/png;base64,',
    thumbUp = 'data:image/png;base64,'
}

type IconProps = {
    iconType: IconType
    className?: string
}

export default function Icon (props: IconProps): JSX.Element {
    function getStylesIcon (): string {
        const color = props.iconType === IconType.thumbUp ? 
            Styles.contentContainer__iconContainer__green :
            Styles.contentContainer__iconContainer__red

        return [Styles.contentContainer__iconContainer, 
            color, props.className].join(' ')
    }
    
    return (
        <div className={getStylesIcon()}>
            <img data-testid="icon" src={props.iconType} />
        </div>
    )
}